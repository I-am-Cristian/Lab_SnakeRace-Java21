...
public final class SnakeApp extends JFrame {
  ...
  public SnakeApp() {
    ...
    this.statusLabel = new JLabel("Presiona Iniciar para comenzar.");
    this.statsLabel = new JLabel("Estado: espera de inicio");

    var controls = new JPanel(new BorderLayout(8, 0));
    controls.add(actionButton, BorderLayout.WEST);
    controls.add(statusLabel, BorderLayout.CENTER);
    controls.add(statsLabel, BorderLayout.EAST);
    ...
    snakes.forEach(s -> {
      var runner = new SnakeRunner(s, board, clock);
      runners.add(runner);
      exec.submit(runner);
    });

    actionButton.addActionListener((ActionEvent e) -> toggleState());
  ...
  private void toggleState() {
    switch (actionButton.getText()) {
      case "Iniciar" -> {
        ...
        statsLabel.setText("Serpientes vivas: " + snakes.size());
      }
      case "Pausar" -> {
        clock.pause();
      ...
      case "Reanudar" -> {
        clock.resume();
        actionButton.setText("Pausar");
        statusLabel.setText("Juego reanudado.");
        statsLabel.setText("Actualizando tablero...");
      }
      default -> { }
    }
  }
  ...
  private void updatePauseStats() {
    var longest = snakes.stream()
        .max(Comparator.comparingInt(s -> s.snapshot().size()))
        .orElse(null);
    var worst = runners.stream()
        .filter(SnakeRunner::isDead)
        .min(Comparator.comparingInt(SnakeRunner::deathOrder))
        .orElse(null);

    String longestText = (longest == null) ? "n/a" : "Larga: " + longest.snapshot().size() + " celdas";
    String worstText = (worst == null) ? "Peor serpiente: ninguna aún" : "Peor serpiente: índice " + worst.deathOrder() + " (" + worst.collisionCount() + " choques)";

    statusLabel.setText("Juego pausado. Estado consistente capturado.");
    statsLabel.setText(longestText + " | " + worstText);
  }
